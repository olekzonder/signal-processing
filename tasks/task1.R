...
task1_server <- function(id){
  moduleServer(
    ...
    function(input,output,session){
      ...
      fm1 <- input$fm1 * 10^3
      output <- c(
      paste("A_0 = U_0 =", input$u0, "\\spaceмВ"),
      paste("T_н = \\frac{1}{f_0} = ", round((1/f0)*10^6,digits=4), "\\spaceмкс"),
      paste("T_м = \\frac{1}{F_м} = ", round(1/fm1*10^6,digits=4), "\\spaceмкс"))

      if (input$dropdown == "Условие 1.1"){
          output <- c(output,c(
              paste("A_{н\\spaceбок} = A_{в\\spaceбок} = \\frac{m_1*U_0}{2} = ", 
              m1*u0/2/10^-3, "\\spaceмВ"),
              paste("\\Delta U =", m1*u0*10^3, "\\spaceмВ"),
      ...
      } else if (input$dropdown == "Условие 1.2") {
              output <- c(output,c(
            paste("A_{н\\spaceбок} = A_{в\\spaceбок} = \\frac{m_1*U_0}{2} = ", 
      m1*u0/2/10^-3, "\\spaceмВ"),
                paste("\\Delta U =", m1*u0*10^3, "\\spaceмВ"),                
              ...
              output <- c(output,c(
                paste("\\Delta U_1=", m1*u0*10^3, "\\spaceмВ"),
                paste("\\Delta U_2=", m1*(u0/2)*10^3, "\\spaceмВ"),
